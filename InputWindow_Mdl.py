...
def get_survey( index ): # FAKE!!!!!!!!!!!!
	
	file_name = 'Survey MWD Oni-1txt.txt'
...
def get_caliper( index ): # FAKE!!!!!!!!!!!!
	...
	return Desc,ID,Drift,MD,FF
	

def calculate_complements( MD, Inc, Azi ): # FAKE!!!!!!!!!!!!
	...
	for i,(md,inc,azi) in enumerate(zip(MD[1:],Inc[1:],Azi[1:])):
	
		TVD.append( TVD[-1] + (md-MD[i])*np.cos( (inc+Inc[i])/2*factor ) )
		HoD.append( HoD[-1] + (md-MD[i])*np.sin( (inc+Inc[i])/2*factor ) )
		NS.append( NS[-1] + (md-MD[i])*np.sin( (inc+Inc[i])/2*factor )*np.cos( (azi+Azi[i])/2*factor ) )
		EW.append( EW[-1] + (md-MD[i])*np.sin( (inc+Inc[i])/2*factor )*np.sin( (azi+Azi[i])/2*factor ) )
		Dogleg.append( 30*(inc-Inc[i])/(md-MD[i]) )
		
	return TVD, HoD, NS, EW, Dogleg 
